...
On startup, reload, resume, or fork, the extension derives prior activity from the active session branch. Ordinary session entries count as observable activity; background extension state and custom context entries do not. The extension writes no session records or mutable configuration of its own.
